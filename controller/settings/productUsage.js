...
exports.insertProductUsage = asyncHandler(async (req, res) => {
  try {
    await sql
      .connect(config)
      .then((pool) => {
        return pool
          ...
          .output("new_identity")
          .execute("settings_product_usage_insert");
      })
      .then((result) => {
        res.send({
          status: 200,
...
exports.browseProductUsage = asyncHandler(async (req, res) => {
  try {
    ...
    await sql
      ...
      .then((pool) => {
        return pool
          ...
          .execute("settings_product_usage_browse");
      })
      .then((result) => {
        const data =
...
//delete
exports.DeleteProductUsage = asyncHandler(async (req, res) => {
  try {
    await sql
      .connect(config)
      .then((pool) => {
        return pool
          ...
          .input("product_usage_id", req.body.product_usage_id)
          .input("user_id",req.user.user_id)
          .execute("settings_product_usage_delete");
      })
      .then((result) => {
        res.send({
          status: 200,
...
exports.previewProductUsage = asyncHandler(async (req, res) => {
  try {
    await sql
      .connect(config)
      .then((pool) => {
        return pool
          .request()
          .input("product_usage_id", req.body.product_usage_id)
          .execute("settings_product_usage_preview");
      })
      .then((result) => {
        res.send({
          status: 200,
